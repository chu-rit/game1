...
function handleGameEnd(game) {
    ...
    // 결과 메시지 표시
    if (player1Won) {
        console.log(playerRole === 'player1' ? '승리했습니다!' : '패배했습니다!');
    } else if (player2Won) {
        console.log(playerRole === 'player2' ? '승리했습니다!' : '패배했습니다!');
    }
...
async function joinGame() {
    ...
    if (matchStatusText) {
        matchStatusText.textContent = '상대방을 찾는 중...';
    }
    ...
    try {
        ...
        console.log('현재 대기실 상태:', waitingGame);

        if (waitingGame) {
            // 두 유저가 동일한 경우
            if (waitingGame.player1 === playerId) {
                console.log('동일한 유저가 매칭되었습니다. 기존 매칭을 지웁니다.');
                ...
                matchStatusText.textContent = '새로운 대기 중...';
                return;
            }
            ...
            if (matchStatusText) {
                matchStatusText.textContent = '게임 시작!';
            }
            gameContainer.classList.remove('hidden');
            isMyTurn = false;
            updateGameState();
            ...
            matchStatusText.textContent = '대기 중...';
        }
        ...
        matchButton.disabled = false;
        if (matchStatusText) {
            matchStatusText.textContent = '오류가 발생했습니다. 다시 시도해주세요.';
        }
    }
}
...
window.onload = function() {
    ...
    if (!matchButton || !gameContainer || !matchStatus || !roundElement || 
        !numberSelectDiv || !playerScoreElement || !opponentScoreElement || !drawScoreElement) {
    ...
    gameContainer.classList.add('hidden');
    if (matchStatusText) {
        matchStatusText.textContent = '게임 시작 대기중...';
    }
}
